"""Grabador de muestras de señas LESCO usando landmarks de MediaPipe."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Graba muestras de señas en formato landmarks.")
    parser.add_argument(
        "--label",
        required=True,
        type=str,
...
def save_sample(label_dir: Path, sample_index: int, frames: List[np.ndarray]) -> Path:
    """Guarda una muestra en disco como archivo .npy con shape (frames, 21, 3)."""
    sample_array = np.array(frames, dtype=np.float32)

    if sample_array.ndim != 3 or sample_array.shape[1:] != (LANDMARKS_PER_HAND, LANDMARK_DIMS):
        raise ValueError(
            "Shape inválido. "
            f"Esperado: (frames, {LANDMARKS_PER_HAND}, {LANDMARK_DIMS}), "
...
def extract_single_hand(
    landmarks: List[List[Tuple[float, float, float]]],
) -> np.ndarray | None:
    """Extrae una mano (la primera detectada) con shape (21, 3)."""
    if not landmarks:
        return None

    first_hand = np.array(landmarks[0], dtype=np.float32)
    if first_hand.shape != (LANDMARKS_PER_HAND, LANDMARK_DIMS):
        return None
...
def main() -> None:
    ...
    try:
        while True:
            ...
            if is_recording:
                one_hand = extract_single_hand(normalized_landmarks)
                if one_hand is not None:
                    current_frames.append(one_hand)
            ...
            if key == ord("s"):
                ...
                output_file = save_sample(label_dir, next_sample_index, current_frames)
                saved_samples += 1
                next_sample_index += 1
                is_recording = False
                current_frames = []
